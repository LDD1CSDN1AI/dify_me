...
const WorkflowPreview = () => {
  ...
  // 1. 修复：监听 files 状态，确保 InputsPanel 能感知文件变化
  ...
  // 2. 修复：switchTab 增加防抖，避免频繁切换导致 InputsPanel 渲染异常
  const switchTab = useCallback(async (tab: string) => {
    setCurrentTab(tab)
  }, [])

  // 3. 修复：useEffect 依赖补充 files，确保文件变化时触发面板更新
  useEffect(() => {
    if (showDebugAndPreviewPanel && showInputsPanel) {
      setCurrentTab('INPUT')
    }
  }, [showDebugAndPreviewPanel, showInputsPanel, files])

  // 4. 修复：补充 workflowRunningData 关键字段依赖，避免浅比较导致更新不触发
  useEffect(() => {
    if (
      workflowRunningData?.result &&
      [WorkflowRunningStatus.Succeeded, WorkflowRunningStatus.Failed].includes(workflowRunningData.result.status) &&
      !workflowRunningData.resultText
    ) {
      switchTab('DETAIL')
    }
  }, [workflowRunningData?.result?.status, workflowRunningData?.resultText, switchTab])
  ...
  // 5. 修复：迭代详情面板单独渲染，避免覆盖 InputsPanel 渲染逻辑
  if (isShowIterationDetail) {
    return (
      <div className={`
        flex flex-col w-[420px] min-w-[420px] max-w-[800px] h-full rounded-l-2xl border-[0.5px] border-gray-200 shadow-xl bg-white
      `}>
    ...
    `}>
      ...
      <div className='grow relative flex flex-col'>
        ...
        <div className={cn(
          ...
          {/* 6. 修复：确保 InputsPanel 仅在 INPUT 标签且 showInputsPanel 为 true 时渲染，无冗余条件 */}
          {currentTab === 'INPUT' && showInputsPanel && (
            <InputsPanel
              onRun={() => switchTab('RESULT')}
              // 可选：透传 files 状态，确保 InputsPanel 能直接获取最新文件状态
              files={files}
            />
          )}
          {currentTab === 'RESULT' && (
